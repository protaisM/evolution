...
template <typename Mouse> class ExperimentDisplay {
...
public:
  ...
  void draw(Experiment<Mouse> const &experiment, sf::RenderWindow *window,
            sf::Vector2f offset, float map_size) const {
    ...
    for (size_t idx_mouse = 0;
         idx_mouse < experiment.m_params.maximal_mice_number; idx_mouse++) {
  ...
  void draw_legend(Experiment<Mouse> const &experiment,
